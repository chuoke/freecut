  ...
  const handleExtractEmbeddedSubtitles = useCallback(async () => {
    ...
    try {
      ...
      if (mediaForItem.storageType === 'handle' && handle) {
        ...
        if (!granted) {
          mediaStore.showNotification?.({
            type: 'error',
            message: `FreeVideoEditor needs permission to read "${mediaForItem.fileName}" before extracting subtitles.`,
          })
          return
        }
      ...
      if (!blob) {
        mediaStore.showNotification?.({
          type: 'error',
          message: `FreeVideoEditor could not load "${mediaForItem.fileName}".`,
        })
        return
      }
      useEmbeddedSubtitlePickerStore.getState().open(mediaForItem, blob)
