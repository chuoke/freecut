// FreeVideoEditor headless single-frame grab CLI.
//
...
async function main() {
  ...
  if (!mime)
    throw new Error(`Unknown --format "${args.format}" (use png|jpg|webp)`);
  const ext = EXT_BY_MIME[mime];
  const outPath = path.resolve(
    args.out ?? path.join("headless", "output", `frame.${ext}`),
  );
...
async function grabFrame(
  page,
  { args, workspace, mediaUrlOf, outPath, mime, frame, atSeconds },
) {
