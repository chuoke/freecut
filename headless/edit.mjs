// FreeVideoEditor headless edit CLI.
//
...
function loadOps(args) {
  ...
  if (!fs.existsSync(opsPath))
    throw new Error(`Ops file not found: ${opsPath}`);
...
async function main() {
  ...
  const ops = validate(editRequestSchema, {
    project: args.project,
    ops: loadOps(args),
  }).ops;
  const { project, projectJsonPath } = loadProject(
    args.workspace,
    args.project,
  );
  if (!args.json)
  ...
  if (missingMeta.length > 0) {
    throw new Error(
      `addClip media not found in workspace: ${missingMeta.join(", ")}`,
    );
  }
  ...
  if (!args.json)
    console.log(
      `Result: ${itemCount} items, ${edited.timeline?.tracks?.length ?? 0} tracks`,
    );
